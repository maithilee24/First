    ...
    return (
        <div className="flex flex-col sm:flex-row md:flex-row lg:flex-row xl:flex-row">
            ...
            <h2 className="font-thin sm:pt-4 text-[1.6rem]
                ...
                <p className=" font-bold pr-2 text-xs text-blue-700 sm:text-black
                    sm:text-xs md:text-[0.8rem] lg:text-[1.1rem] xl:text-[1.4rem]">{props.linkContent}</p>
                <img className="ml-2 my-2 w-2 h-1
                    sm:w-4 sm:h-2 
                    md:w-6 md:h-2 
                    lg:w-8 lg:h-[0.6rem] 
                    xl:w-12 xl:h-[0.7rem]" src="images/Arrow.svg"/>
                </div>
                </Link>
            </div>
            <div className=" hidden sm:block md:block lg:block xl:block
            w-1/2 ">
                <img src={props.img} className=" sm:h-auto xl:max-h-[22rem]"  style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
        </div>
    );
}
